...
- A page for adding or editing transaction information, which does not accept invalid information
...
- Some form of visualization like a pie chart representing users expenses/income 
...
### Other Repositories
This application can be run in a standalone fashion, but nothing related to database/API connections (Adding/editing transactions) will function properly. The following steps must be done before attempting to compile this project if actual interaction with the database wants to be obtained:
...
- Clone this repository using `git clone https://github.com/WSU-kduncan/cs4900-ui-wolf-of-wright-street`, navigate into the repository and run the command `cd wolf-budget-ui-app` to get to the proper folder.
- Install all required dependencies listed in `package.json` using `npm install`.
- Run the application by running command `ng serve`.
